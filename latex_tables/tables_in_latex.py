...
root_path = os.getcwd()
costs = pd.read_csv(
    os.path.join(root_path, "outputs", "costs_2060.csv"), index_col=list(range(2))
).sort_index()
...
dic_ref = {
    "Technology Data for Energy Plants for Electricity and District heating generation": "DEA_2019",
...
for technology in technologies:
    ...
    if idx[technology, "investment"] in costs.index:
        ...
        investment = " "
    if idx[technology, "efficiency"] in costs.index and technology not in [
        "onwind",
        "offwind",
        "central gas CHP",
        "biomass CHP",
...
dic_units = {
    "EUR/kWel": r"\EUR/kW$_{el}$",
    "EUR/kWth": r"\EUR/kW$_{th}$",
    "EUR/kWH2": r"\EUR/kW$_{H_2}$",
    "EUR/kW_CH4": r"\EUR/kW$_{CH4}$",
    "EUR/kWCH4": r"\EUR/kW$_{CH4}$",
    "EUR/kWhth": r"\EUR/kWh$_{th}$",
    "EUR/(tCO2/a)": r"\EUR/(tCO$_2$/a)",
    "EUR/(tCO2/h)": r"\EUR/(tCO$_2$/h)",
    "EUR/m3": r"\EUR/m$^3$",
    "EUR/MW/km": r"\EUR/MWkm",
    "EUR/MW": r"\EUR/MW",
    "USD/kWel": "USD/kW$_{el}$",
    "USD/kWh": "USD/kWh",
    "EUR/kWh": r"\EUR/kWh",
    "EUR/kW": r"\EUR/kW",
    "EUR/kW_e": r"\EUR/kW$_{el}$",
    "EUR/kW_th - heat output": r"\EUR/kW$_{th}$",
    "EUR/kW_th": r"\EUR/kW$_{th}$",
    "EUR/kWhCapacity": r"\EUR/kWh",
    "EUR/kW_th excluding drive energy": r"\EUR/kW$_{th}$",
    "EUR/kW_FT/year": r"\EUR/kW$_{FT}$/a",
    "EUR/kW_MeOH": r"\EUR/kW$_{MeOH}$",
}
...
for technology in technologies:
    ...
    for year in years:
        costs_year = pd.read_csv(
            "../outputs/costs_" + str(year) + ".csv", index_col=list(range(2))
        ).sort_index()
        if technology in ["hydrogen storage underground", "central water tank storage"]:
            file.write(
                str(round(costs_year.loc[idx[technology, "investment"], "value"], 1))
                + " & "
            )
        else:
            file.write(
                str(int(costs_year.loc[idx[technology, "investment"], "value"])) + " & "
            )
