...
import numpy as np
import pandas as pd
from compile_cost_assumptions import (adjust_for_inflation, clean_up_units,
                                      convert_units, get_data_DEA,
                                      get_data_from_DEA, get_excel_sheets,
                                      get_sheet_location, new_format,
                                      sheet_names, source_dict,
                                      uncrtnty_lookup)
...
def get_data_from_DEA(data_in, sheet_names, expectation=None):
    ...
    for tech, dea_tech in sheet_names.items():
        ...
        df = get_data_DEA(tech=tech, data_in=data_in, sheet_names=sheet_names, expectation=expectation).fillna(0)
        d_by_tech[tech] = df
...
if __name__ == "__main__":
    if 'snakemake' not in globals():
        ...
        snakemake = mock_snakemake("compile_intl_cost_assumptions")
    ...
    d_by_tech = get_data_from_DEA(data_in=data_in, expectation=snakemake.config["expectation"],sheet_names=dea_indonesia)

    tech_data = pd.concat(d_by_tech).sort_index()
    # clean up units
    tech_data = clean_up_units(tech_data, years)

    tech_data = tech_data.loc[(tech_data[years] != 0).sum(axis=1) != 0]

    tech_data = add_mean_solar_rooftop(tech_data)
    ...
    for year in years:
        ...
        costs.loc[:,'value'] = round(costs.value.astype(float), snakemake.config.get("ndigits", 2))
